...
def split_excel_file(input_file: Path, max_workers: int = None) -> bool:
    ...
    try:
        ...
        # 准备所有任务参数
        tasks = [
            (str(input_file), sheet_name, base_name, output_dir)
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任务
            future_to_sheet = {
                executor.submit(process_single_sheet, task): task[1] 
                for task in tasks
            }
            ...
            for future in as_completed(future_to_sheet):
                ...
                if success:
                    success_count += 1
                    show_success(
                        f"[{success_count}/{total_sheets}] "
                        f"已保存工作表 '{sheet_name}' 到 '{output_file_name}'"
                    )
                else:
