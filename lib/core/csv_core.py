...
def txt_to_csv(input_file: Path, output_file: Path | None = None) -> bool:
    """
    TXT 转 CSV
    自动检测分隔符
    """
...
def csv_to_txt(input_file: Path, output_file: Path | None = None, delimiter: str = '\t') -> bool:
    """
    CSV 转 TXT
    默认使用 Tab 分隔
    """
...
def merge_txt_files(directory: Path, output_file: Path | None = None) -> bool:
    """
    合并目录中所有 TXT 文件为 CSV
    按文件名中的数字排序
    """
...
def reorder_csv(input_file: Path, order_file: Path, output_file: Path | None = None) -> bool:
    """
    根据列表文件重排 CSV 行
    """
    if output_file is None:
...
def format_circles(input_file: Path, output_file: Path | None = None) -> bool:
    """
    格式化带圈数字
    将 ① ② ③ 等转换为 1. 2. 3. 等
    """
