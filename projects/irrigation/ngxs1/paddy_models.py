...
from config import log, LOG_CONFIG, WARMUP_DAYS
...
class IrrigationArea:
    ...
    def get_flowering_periods(self):
        """获取开花期配置"""
        # 动态导入配置，避免循环引用
        from config import FLOWERING_PERIODS
...
class IrrigationSimulator(SimulationBase):
    ...
    def run(self):
        ...
        start_time = pd.Timestamp(engine.current_time) - pd.Timedelta(days=WARMUP_DAYS)
        time_range = pd.date_range(
            start_time, 
            periods=engine.forecast_days + WARMUP_DAYS, 
            freq='D'
        )
