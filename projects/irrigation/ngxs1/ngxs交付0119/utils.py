...
def read_data_file(file_path: str, debug: bool = False, skip_comments: bool = True) -> List[str]:
    """
    读取数据文件内容，返回非空行列表
    ...
    参数:
        file_path: 文件路径
        debug: 是否输出详细调试信息
        skip_comments: 是否跳过以 # 开头的注释行（默认 True）
...
def export_results(
    ...
    """
    # 使用重构后的配置模块
    from config import OUTPUT_DIR, WARMUP_DAYS
    
    data = {}
    
    # 如果包含预热期数据，则从预热期开始计算
    if include_warmup_days:
        start_time = pd.Timestamp(current_time) - pd.Timedelta(days=WARMUP_DAYS)
