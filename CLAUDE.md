...
├── tools/            # 项目维护工具（health_check, sync_index, cf_api, gen_claude_md, vps_cmd, validate_raycast）
├── asr/              # 语音识别词表数据
...
templates/            # 模板文件（gitignored，不入库）

raycast/              # Raycast 入口
├── commands/         # Shell wrapper 脚本（统一入口）
└── lib/              # 运行器（run_python.sh → 调用 lib/common.sh）

repo_manager.py       # GitHub 仓库管理（promote/audit/screenshot 子命令）
```
...
### 命名规则
- 脚本保留功能前缀：`docx_`、`xlsx_`、`csv_`、`md_`、`pptx_`、`clashx_`
