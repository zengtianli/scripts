...
class DataManager:
    ...
    def get_static_file_path(self, crop_type: str) -> str:
        ...
        返回:
            文件完整路径
        """
        if crop_type in STATIC_IRRIGATION_FILES:
            return os.path.join(self.data_path, STATIC_IRRIGATION_FILES[crop_type])
        return None
...
class IrrigationManager:
    ...
    def initialize_systems(self, current_time: datetime, forecast_days: int):
        ...
        """
        # 设置历史数据起点（预热期）
        start_time = current_time - pd.Timedelta(days=WARMUP_DAYS)
...
class ResultExporter:
    ...
    def export_results(self, data_path: str, current_time: datetime, forecast_days: int, return_data=False):
        ...
        # 添加逐作物逐日数据
        if return_data:
            results['daily_by_crop'] = self._build_daily_by_crop(area_names, current_time, forecast_days)
        
        return results
    
    def _build_daily_by_crop(self, area_names: List[str], current_time: datetime, forecast_days: int) -> Dict:
        """
        构建各作物的逐日数据
        ...
        for day in range(forecast_days):
            ...
            for area_name in area_names:
                ...
                # 单季稻灌溉
                if 'single_crop_irrigation' in area.results and current_date in area.results['single_crop_irrigation']:
                    single_total += area.results['single_crop_irrigation'][current_date]
                
                # 双季稻灌溉
                if 'double_crop_irrigation' in area.results and current_date in area.results['double_crop_irrigation']:
                    double_total += area.results['double_crop_irrigation'][current_date]
            
            if single_total > 0:
                daily_data[date_str]['单季稻'] = single_total
            if double_total > 0:
                daily_data[date_str]['双季稻'] = double_total
            
            # 2. 获取旱地作物数据（从 dryland_daily_by_crop 中）
            if hasattr(self.calculator, 'dryland_daily_by_crop') and date_str in self.calculator.dryland_daily_by_crop:
                for crop_name, water in self.calculator.dryland_daily_by_crop[date_str].items():
                    if water > 0:
                        daily_data[date_str][crop_name] = water
...
class Calculator:
    ...
    def __init__(self, data_path: str, verbose: bool = False):
        ...
        # 旱地作物逐日数据（由 CropSimulator.run 填充）
        self.dryland_daily_by_crop = {}
