...
class DataOutputProcessor:
    ...
    def generate_16_hq_summary():
        """生成15个河区汇总文件"""
        print("开始生成15个河区数据汇总文件...")
        columns_to_sum = SUMMARY_COLUMNS
        base_dir = str(Config.BASE_DIR)

        # 明确列出要汇总的15个河区文件
        hq_16_files = [
            'output_hq_SSPYQ.txt',      # 蜀山平原区
            'output_hq_NSPYQ.txt',      # 南沙平原区
            'output_hq_SYPYQ.txt',      # 绍虞平原区
            'output_hq_YBPYSHQ.txt',    # 虞北平原上河区
            'output_hq_YBPYZHQ.txt',    # 虞北平原中河区
            'output_hq_YYPYSHQ.txt',    # 余姚平原上河区
            'output_hq_YYPYXHQ.txt',    # 余姚平原下河区
            'output_hq_YYPYMZZHQ.txt',  # 余姚平原马渚中河区
            'output_hq_CXPYXHQ.txt',    # 慈溪平原西河区
            'output_hq_CXPYZHQ.txt',    # 慈溪平原中河区
            'output_hq_CXPYDHQ.txt',    # 慈溪平原东河区
            'output_hq_FHPYQ.txt',      # 丰惠平原区
            'output_hq_YYPYYJSYQ.txt',  # 余姚平原姚江上游区
            'output_hq_YYPYYJXYQ.txt',  # 余姚平原姚江下游区
            'output_hq_JBZHPYQ.txt'     # 江北镇海平原区
        ]
        ...
        print(f"找到 {len(hq_files)} 个河区数据文件（共15个）")
        ...
        output_file = os.path.join(base_dir, 'output_hq_all.txt')
        summary_df.to_csv(output_file, sep=Config.FILE_SEPARATOR, index=False)
        print(f"15个河区每日汇总数据已保存到：{output_file}")
        ...
        cumulative_output_file = os.path.join(base_dir, 'output_hq_all_cumulative.txt')
        cumulative_df.to_csv(cumulative_output_file, sep=Config.FILE_SEPARATOR, index=False)
        print(f"15个河区累积汇总数据已保存到：{cumulative_output_file}")
        ...
        total_output_file = os.path.join(base_dir, 'output_hq_all_total.txt')
        total_df.to_csv(total_output_file, sep=Config.FILE_SEPARATOR, index=False)
        print(f"15个河区总计汇总（{start_date}至{end_date}）已保存到：{total_output_file}")
    ...
    @staticmethod
    def append_summary_row_to_district_files():
        """
        为每个河区输出文件追加合计行。
        只处理 output_hq_*.txt 文件，排除 output_hq_all*.txt 等汇总文件。
        """
        import glob
        ...
        # 获取所有 output_hq_*.txt 文件，排除包含 "all" 的文件
        pattern = os.path.join(base_dir, "output_hq_*.txt")
        all_files = glob.glob(pattern)
        district_files = [f for f in all_files if "all" not in os.path.basename(f)]

        print(f"找到 {len(district_files)} 个河区文件需要处理")

        for file_path in district_files:
            try:
                filename = os.path.basename(file_path)
                df = pd.read_csv(file_path, sep=Config.FILE_SEPARATOR, encoding='utf-8')
                ...
                summary_row = {}
                for col in df.columns:
                    if col == Config.DATE_COLUMN or col == '日期':
                        # 日期列显示"合计"
                        summary_row[col] = '合计'
                    elif col in ['日初容积', '初河蓄水']:
                        # 取第一天的值
                        summary_row[col] = df[col].iloc[0]
                    elif col == '排末容积':
                        # 取最后一天的值
                        summary_row[col] = df[col].iloc[-1]
                    else:
                        # 其他数值列求和
                        try:
                ...
                # 写回文件
                df_with_summary.to_csv(file_path, sep=Config.FILE_SEPARATOR, index=False, encoding='utf-8')
                print(f"  ✓ {filename}")
...
class WaterResourcesManager:
    ...
    def run():
        ...
        DataOutputProcessor.append_summary_row_to_district_files()  # 为河区文件添加合计行
        print("\n处理完成！")
def main():
