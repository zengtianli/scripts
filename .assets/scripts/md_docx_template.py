...
# @raycast.description Markdown 转 Docx（样式复刻版）
# -*- coding: utf-8 -*-
"""
...
def extract_styles_xml(docx_path, output_dir=None):
    ...
    for style in root.findall('.//w:style', NSMAP):
        ...
        if style_id in TARGET_STYLES:
            collected_styles[style_id] = {
                'element': deepcopy(style),
                'name': style_name,
                'id': style_id
            }
...
def parse_markdown(md_content):
    ...
    while i < len(lines):
        ...
        if is_list_start:
            ...
            while i < len(lines):
                ...
                is_list_item = (
                    ...
                    (current_stripped.startswith('* ') and not current_stripped.startswith('**')) or 
                    current_stripped.startswith('> ') or
                    re.match(r'^\d+\.\s', current_stripped) or
                    (current.startswith('  ') and list_items)  # 缩进的续行
                )
        ...
        while i < len(lines):
            ...
            # 特殊行结束段落
            if (re.match(r'^#{1,4}\s', next_line) or  # 标题
                next_stripped.startswith('|') or       # 表格
                next_stripped.startswith('- ') or      # 列表
                (next_stripped.startswith('* ') and not next_stripped.startswith('**')) or
