...
type_colors = {"工作任务": "#2196F3", "里程碑": "#FF5722", "子任务": "#9C27B0", "假期": "#CCCCCC"}
...
def plot_complete_gantt(df):
    ...
    for i, (_idx, task) in enumerate(df.iterrows()):
        if task["类型"] == "里程碑":
            # 里程碑用菱形标记
            ax.scatter(
                task["开始"], i, marker="D", s=200, color=task["类型颜色"], zorder=3, edgecolor="black", linewidth=2
            )
            ax.text(task["开始"], i - 0.3, "★", ha="center", va="center", fontsize=16, color="gold", weight="bold")
        else:
            # 普通任务用条形图
            ax.barh(
                i, task["工期"], left=task["开始"], height=0.6, color=task["阶段颜色"], alpha=0.8, edgecolor="black"
            )

            # 在条形图上添加任务名称
            if task["工期"] > 3:  # 只在足够长的任务上显示文字
                ax.text(
                    task["开始"] + pd.Timedelta(days=task["工期"] / 2),
                    i,
...
def plot_main_tasks_gantt(df):
    ...
    for i, (_idx, task) in enumerate(main_tasks_df.iterrows()):
        ...
        ax.text(
            mid_date,
            i,
            f"{task['开始'].strftime('%m/%d')}-{task['结束'].strftime('%m/%d')}",
            ha="center",
            va="center",
            fontsize=11,
            weight="bold",
...
def plot_milestones_timeline(df):
    ...
    # 绘制时间线
    y_pos = 0
    ax.plot([milestones_df["开始"].min(), milestones_df["开始"].max()], [y_pos, y_pos], "k-", linewidth=4, alpha=0.3)
    ...
    for i, (_idx, milestone) in enumerate(milestones_df.iterrows()):
        ...
        # 添加里程碑标号
        ax.text(
            milestone["开始"], y_pos, str(i + 1), ha="center", va="center", fontsize=14, weight="bold", color="white"
        )
...
def plot_phase_distribution(df):
    ...
    wedges, texts, autotexts = ax1.pie(
        phase_duration.values,
        labels=phase_duration.index,
        autopct="%1.1f%%",
        colors=colors,
        startangle=90,
    ...
    bars = ax2.bar(range(len(task_count)), task_count.values, color=[phase_colors[phase] for phase in task_count.index])
...
def plot_project_dashboard(df):
    ...
    ax2.barh(range(len(milestones)), [1] * len(milestones), color=milestone_colors[: len(milestones)])
    ...
    ax3.pie(
        phase_duration.values,
        labels=list(phase_duration.index),
        autopct="%1.0f天",
        colors=[phase_colors[p] for p in phase_duration.index],
    )
    ...
    for i, (_, row) in enumerate(key_dates.iterrows()):
        ax6.text(0.1, 0.7 - i * 0.2, f"{row['开始'].strftime('%m-%d')}: M{i + 1}", fontsize=10, transform=ax6.transAxes)
    ax6.axis("off")
