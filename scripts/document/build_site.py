...
CATEGORY_COLORS = {
    "sessions": "#3b82f6", "knowledge": "#10b981", "memory": "#8b5cf6",
    "projects": "#f59e0b", "architecture": "#06b6d4", "discussions": "#f97316",
    "decisions": "#ef4444", "_archive": "#6b7280", "guides": "#14b8a6",
    "cc-memory": "#a855f7", "cc-sessions": "#0ea5e9",
}
...
STOP_WORDS = {
    "the", "is", "and", "to", "of", "a", "in", "for", "on", "it",
    "that", "with", "as", "this", "was", "are", "be", "at", "from", "or",
    "an", "by", "not", "but", "have", "has", "had", "do", "does", "did",
    "will", "would", "could", "should", "may", "might", "can", "been",
    "being", "were", "which", "their", "what", "there", "when", "than",
    "other", "its", "into", "some", "these", "them", "then", "two",
    "no", "my", "more",
}
...
def collect_cc_memory(cc_dir):
    ...
    for md_path in sorted(projects_dir.rglob('memory/*.md')):
        ...
        memories.append({
            ...
            'body': body,
            'html_rel': html_rel,
            'tags': [t.strip() for t in str(meta.get('tags', '')).split(',') if t.strip()] if meta.get('tags') else [],
...
def render_dashboard(docs, projects, stats, similar, build_time,
                     cc_memories=None, cc_sessions=None, duplicates=None):
    """渲染 Dashboard 首页。"""
    # --- Header ---
  ...
  {''.join(mem_cards)}
    ...
    if cc_sessions:
        ...
        cat_colors = {
            '开发': '#10b981', '配置': '#3b82f6', '学习': '#8b5cf6',
            '排查': '#ef4444', '写作': '#f59e0b', '整理': '#06b6d4',
            '讨论': '#f97316',
        }
        ...
        for s in show_sessions:
            ...
            summary_html = ''
            if summary:
                summary_html = f'<div style="font-size:0.85em;color:var(--text-secondary);margin-top:3px">{summary}</div>'
        ...
        if len(valid_sessions) > 20:
            ...
            for s in remaining_sessions:
                ...
                sessions_for_js.append({
                    'project': s.get('project', '').replace('-Users-tianli-', '').replace('-Users-tianli', '~') or '~',
                    'title': s.get('title', 'Untitled')[:60],
...
def render_cc_memory_detail(mem, build_time):
    ...
    # Breadcrumb
    bc = (f'<a href="{root_prefix}index.html">Dashboard</a>'
...
def render_cc_session_detail(session, build_time):
    ...
    rows_html = '\n'.join(
        f'<tr><td style="font-weight:600;width:160px">{k}</td><td>{v}</td></tr>'
...
def render_graph_page(docs, edges, stats, build_time):
    ...
    for cat in all_cats:
        ...
        checkboxes.append(f'<label style="display:flex;align-items:center;gap:6px;font-size:0.85em;cursor:pointer">'
...
def render_directory_index(dir_rel, files_in_dir, subdirs, docs_by_path, build_time):
  ...
  <span class="file-date">{date_str}</span>
        ...
        sections.append(f'<div class="dir-section-title">Files ({len(files_in_dir)})</div><ul class="file-list">{"".join(items)}</ul>')
...
def build_site(src_dir, output_dir, cc_dir=None):
    ...
    # 3a. Dashboard
    dashboard_html = render_dashboard(docs, projects, stats, similar, build_time,
                                      cc_memories=cc_memories, cc_sessions=cc_sessions,
